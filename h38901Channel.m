...
%   Copyright 2022-2024 The MathWorks, Inc.
    ...
    methods (Access=public)
        ...
        % Create channel model
            ...
            channel.theUEMap = dictionary([],struct(Node={},NodeSubs={},n_fl={},d_2D_in={}));

            % Create map between IDs and BSs
            channel.theBSMap = dictionary([],struct(Node={},OFDMInfo={}));
        ...
        % all gNBs or all UEs as appropriate.
            ...
            % hidden properties InterfererHasSmallScale and PhaseLOS_d_3D
            % to be controlled
        ...
        % simulator.
            ...
            % Check if packet Abstraction is true
            if (~packet.Abstraction)
                error('nr5g:h38901Channel:FullPHYUnsupported','Only abstract PHY is supported.');
            end
            ...
            [ch,bsID] = getSLSChannel(obj,rxinfo,packet);
            % -------------------------------------------------------------

            % If the channel is empty, signifying a BS-to-BS or UE-to-UE
            % link, return an empty packet that will be dropped by the
            % simulator
            if (isempty(ch))
                packet = [];
                return;
            ...
            if (~isstruct(ch.SmallScale))
                ...
                pathDelays = channelInfo.PathDelays;
                ...
                % slot
                [pathGains,sampleTimes] = spanSlot(obj,bsID,pathGains,sampleTimes);
                ...
                pathGains = h38901Channel.applyBeamforming(ch.SmallScale,pathGains,ch.TXRUVirtualization);
                pathDelays = ch.SmallScale.PathDelays;
                pathFilters = ch.PathFilters;
                sampleTimes = 0;
...
function [channel,bsID] = getSLSChannel(obj,rxinfo,packet)
    ...
    % UE-to-UE link, return an empty channel
    ...
    channel = getSLSChannelLink(obj,thisLinkID,otherLinkID,linkID,rxinfo,packet,isUplink);
...
function channel = getSLSChannelLink(obj,thisLinkID,otherLinkID,linkID,rxinfo,packet,isUplink)
    ...
    % TR 38.901 Section 7.5 Steps 2 - 10
    if (isempty(channel))
        channel = createSLSChannelLink(obj,linkID,packet,isUplink);
    end
    % ---------------------------------------------------------------------
...
function channel = createSLSChannelLink(obj,linkID,packet,isUplink)
    ...
    if (isUplink)
        % Note that channels are in the downlink direction and operate as
        % reciprocal links for the uplink direction. Therefore, the antenna
        % counts below are for the downlink
    ...
    chcfg.NodeSubs = [BS.NodeSubs(1:2) UE.NodeSubs(3)];
    chcfg.NodeSiz = obj.theScenarioInfo.NodeSiz;
...
function recordNodes(obj,gNBs,UEs,chCfg)
    ...
    if (~isempty(maxBSID) && ~isempty(maxUEID))
        if (maxUEID < maxBSID)
            error('nr5g:h38901Channel:UENodeFirst','The maximum UE ID (%d) is less than maximum gNB ID (%d). Create all gNB nodes before creating any UE nodes, to ensure that all UE IDs are greater than all gNB IDs.',maxUEID,maxBSID);
        end
    end
    ...
    for i = 1:numel(gNBs)
        ...
        for j = 1:numel(UEs{i})
            ...
            obj.theUEMap(ue.ID) = struct(Node=ue,NodeSubs=subs,n_fl=n_fl,d_2D_in=d_2D_in);
...
function [channel,chinfo,chCfg,siteuers,SCRVs_out] = fastFadingChannelModel(chCfg)
    ...
    if (chCfg.HasSmallScale)
        ...
        % Configure empty small scale channel and path filters
        ss = [];
        pathFilters = [];
...
function [pathGains,sampleTimes] = spanSlot(obj,bsID,pathGains,sampleTimes)

    if (~isscalar(sampleTimes))

        % Calculate sample time Ts
        BS = obj.theBSMap(bsID);
        ofdmInfo = BS.OFDMInfo;
        ...
        wholeSubframes = floor(sampleTimes(1) / 1e-3);
...
function pathgains = noFastFadingPathGainsLocal(ch)
    ...
    % If the function is called with no input arguments, clear the
